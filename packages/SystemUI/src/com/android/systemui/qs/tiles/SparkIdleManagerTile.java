...
public class SparkIdleManagerTile extends SecureQSTile<QSTile.BooleanState> {

    private final Icon mIcon = ResourceIcon.get(R.drawable.ic_spark);
    ...
    public SparkIdleManagerTile(
        ...
        mSetting = new SettingObserver(secureSettings, mHandler, Settings.Secure.SPARK_AGGRESSIVE_IDLE_MODE) {
            @Override
    ...
    private void setEnabled(boolean enabled) {
        Settings.Secure.putInt(mContext.getContentResolver(),
                Settings.Secure.SPARK_AGGRESSIVE_IDLE_MODE, enabled ? 1 : 0);
    }
    ...
    protected void handleUpdateState(BooleanState state, Object arg) {
        ...
        state.label = mContext.getString(R.string.quick_settings_spark_idle_mode_label);
        state.icon = mIcon;
        state.contentDescription = TextUtils.isEmpty(state.secondaryLabel)
                ? state.label
                : TextUtils.concat(state.label, ", ", state.secondaryLabel);
	state.secondaryLabel = sysManagerEnabled ? mContext.getResources().getString(sysIdleManagerEnabled ? R.string.quick_settings_spark_idle_mode_enabled : R.string.quick_settings_spark_idle_mode_disabled) : mContext.getResources().getString(R.string.quick_settings_spark_services_unavailable);
